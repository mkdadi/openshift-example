...
**Enhancements**
- **Accordion** - Accordion can now specify a trigger element instead of `title`, added an [example in docs](//www.semantic-ui.com/modules/accordion.html#changing-trigger)
- **Accordion** - Accordion can now hide while opening animation is still occuring
- **Grid** - Equal width grids will now make column content stretch to full height, not just the column itself (requires flexbox). See examples [in the grid docs](//www.semantic-ui.com/collections/grid.html#equal-height)
...
[See the examples](//www.semantic-ui.com/behaviors/visibility.html#/examples) online for a demonstration.
...
**New Features**
- **Transition** - Transitions now have `interval` to allow grouped elements to animate one by one with a delay between each animation. Grouped animations determine order based on transition direction to avoid reflows, or can manually be reversed by using <code>reverse: true</code> [See Examples](//www.semantic-ui.com/modules/transition.html#grouped-transitions) for more details.
...
- **Reset** - Reset now inherits ``box-sizing`` [from html tag](//css-tricks.com/inheriting-box-sizing-probably-slightly-better-best-practice/)
